...
//   invalidCandidates  run1Success(total)  run2Success(total)
...
function main() {
    ...
    try {
        const protocol = readJson(path.join(smokeRoot, 'protocol.json'));
    ...
    const runs = report.threadFetch.runs.map(run => {
        ...
        const successes = Array.isArray(run.observations)
