...
## Changes recommended to the current documentation

1. Amend [ADR-002](../adr/002-pydantic-ai-primary-orchestrator.md) so the decision says **“primary typed LLM execution layer”**, with agent/tool orchestration as one use case rather than the whole scope.
2. Keep [ADR-005](../adr/005-bounded-agents-deterministic-workflows.md) unchanged in principle: deterministic application workflows remain the control plane.
3. Extend [AI and Agent Design](../architecture/agents.md) with a task catalog, per-task input/output schemas, prompt/schema versioning, model policy, retry budget, usage budget and evaluation suite.
4. State explicitly in [Human Approval and Security](../architecture/approval-security.md) that Pydantic AI deferred-tool approval may improve interaction flow but cannot authorize publishing.
5. Add the Pydantic AI run ID/conversation ID, actual provider/model, model settings, token/cost usage, retry count, prompt version and schema version to model-run provenance. Do not store full prompts/completions by default without a retention and redaction policy.
6. Pin a Pydantic AI v2-compatible dependency range. Stable v2 was released on 2026-06-23; minor releases are intended to avoid breaking changes, but new message/event variants and OpenTelemetry attributes may change, and APIs under `beta` are explicitly not stable. ([Version policy](https://pydantic.dev/docs/ai/project/version-policy/))
