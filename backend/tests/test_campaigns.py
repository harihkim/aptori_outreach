...
def test_create_validation_rejects_blank_names(client: TestClient) -> None:
    response = client.post("/campaigns", json=create_payload(name="   "))

    assert response.status_code == 422


def test_create_validation_rejects_unknown_posture(client: TestClient) -> None:
    response = client.post("/campaigns", json=create_payload(promotion_posture="aggressive"))

    assert response.status_code == 422


def test_create_contract_is_closed_to_unknown_fields(client: TestClient) -> None:
    response = client.post("/campaigns", json=create_payload(surprise="value"))
...
def created_campaign(client: TestClient, **overrides: Any) -> dict[str, Any]:
    response = client.post("/campaigns", json=create_payload(**overrides))
    assert response.status_code == 201
...
def test_foreign_workspace_campaigns_are_invisible(
    ...
    assert (
        client.patch(f"/campaigns/{foreign_campaign}", json={"name": "x"}).status_code
        == 404
    )
...
def test_patch_contract_rejects_unknown_fields(client: TestClient) -> None:
    ...
    response = client.patch(f"/campaigns/{created['id']}", json={"surprise": 1})
...
def test_lifecycle_walks_the_legal_path_and_audits_transitions(
    ...
    for target in ("active", "paused", "active", "archived"):
        response = client.patch(f"/campaigns/{campaign_id}", json={"status": target})
        assert response.status_code == 200
...
def test_illegal_transitions_rejected_with_stable_code(client: TestClient) -> None:
    ...
    assert (
        client.patch(f"/campaigns/{campaign_id}", json={"status": "archived"}).status_code
        == 409
    )
...
def test_archived_campaign_is_read_only(client: TestClient) -> None:
    ...
    for target in ("active", "archived"):
        assert (
            client.patch(f"/campaigns/{campaign_id}", json={"status": target}).status_code
            == 200
        )

    field_edit = client.patch(f"/campaigns/{campaign_id}", json={"name": "X"})
    reopen = client.patch(f"/campaigns/{campaign_id}", json={"status": "active"})
    same_status = client.patch(f"/campaigns/{campaign_id}", json={"status": "archived"})
...
def test_same_status_patch_is_an_idempotent_noop(
    ...
    campaign_id = created["id"]
    assert (
        client.patch(f"/campaigns/{campaign_id}", json={"status": "active"}).status_code
        == 200
    )
    before_events = audit_events(migrated_test_database, campaign_id)

    response = client.patch(f"/campaigns/{campaign_id}", json={"status": "active"})
